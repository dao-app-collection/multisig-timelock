...
const Transaction = (props) => {
  ...
  const getProviderOrSigner = async (needSigner = false) => {
    ...
    if (chainId !== 80001) {
      window.alert("Change the network to Polygon Mumabai");
      throw new Error("Change network to Polygon Mumabai");
    }
    if (needSigner) {
  ...
  const handleClick = () => {
    window.open(
      `https://mumbai.polygonscan.com/address/${props.contractAddress}`
    );
  };
